...
int DoSomethingToFlynn(int flynn)
{
    double age,dislocden;
	
	ElleGetFlynnRealAttribute(flynn, &age, AGE); // get attribute for this flynn
    age++;
	ElleSetFlynnRealAttribute(flynn, age,AGE); // set attribute for this flynn
    ElleSetFlynnRealAttribute(flynn,exp(-1.0/age),DISLOCDEN); // set attribute for this flynn
