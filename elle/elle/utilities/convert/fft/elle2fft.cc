...
main(int argc, char **argv)
{
    ...
    userdata[SQ_patterns]=0; // default grid pattern is square (overridden in code)
        ...
        for (c=0;c<cols;c++,n++) {
            ...
            if (i!=NO_VAL) {
               ...
               //  it is not reset in the 2nd write block below
                ...
                // Flynn dislocation density is used as phase indicative
                ElleGetFlynnRealAttribute(jgrain,&jphase,DISLOCDEN);
    ...
    for (r=1;r<=rows && outf;r++) {   // cycle through fft points
        ...
        for (c=1;c<=cols && outf;c++,i++) {
            ...
            if (jphase > INT_MAX || jphase < INT_MIN)
              ...
              pts_i[i]= static_cast<int> (jphase);
