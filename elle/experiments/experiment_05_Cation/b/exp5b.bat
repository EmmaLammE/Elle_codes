...
cd %ELLEPATH%\..\experiments\experiment_05_Cation\b
TITLE=exp5b
set DELETE=del /Q
set COPY=copy /y
set MOVE=rename
set BIN=%ELLEPATH%\binwx
set STARTFILE=exp5b.elle
set T=0
set LASTTIME=1200
set TIMESTEP=1
set GGSTAGES=000
...
set U5=
set U6=
set GBDIFFSTAGES=1000
set GBDIFFKAPPA=2e-9
set LATTDIFFSTAGES=000
set EXCHSTAGES=500
set EXCHKAPPA=30.90528
set EXCHTEMP=-0.5
set EXCHDIFFONLY=0
set EXPANDAREA=0
set EXPANDSPEED=10
...
set SAVEINTVL=200
set SAVEALL=15
set BASINFILE=intilt3
set ROOTNAME=exp5b_out
set GV_SAVE=0
set PLOTAX=0
set LIST=20 23
set R=%ROOTNAME%
...
echo %BIN%\elle_exchange -i tmp.elle -s !S! -f 001 !P! -n
%DEBUG% start /w %BIN%\elle_exchange -i tmp.elle -s !S! -f 001 !P! -n
if NOT ERRORLEVEL 0 GOTO END
echo %COPY% exchange001.elle tmp.elle
%DEBUG% %COPY% exchange001.elle tmp.elle
if NOT ERRORLEVEL 0 GOTO END
if %CNT% EQU 0 (
   echo %MOVE% exchange001.elle %R%.%TSTR%.23.elle
   %DEBUG% %MOVE% exchange001.elle %R%.%TSTR%.23.elle
) ELSE (
   if %SAVEALL% EQU 23 %DEBUG% %MOVE% exchange001.elle %R%.%TSTR%.23.elle
   )
)
